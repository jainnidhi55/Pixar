...
var movies = [
    ...
    new Movie("Brave", "Her hair was really hard to animate.", "images/brave.jpg", 2011),
    new Movie("Cars 2", "So. Many. Explosions.", "images/cars2.jpg", 2012),
...
function sortMoviesByYear(movies) {
    ...
    for (var i = 0; i<movies.length-1; i++){
		...
		for (var j = i+1; j<movies.length; j++){
			if (movies[j].year > movies[maxIndex].year){
				var temp = movies[j];
				movies[j] = movies[maxIndex];
...
function sortMoviesByTitle(movies) {
    ...
    for (var i = 0; i<movies.length-1; i++){
		var maxIndex = i;
		for (var j = i+1; j<movies.length; j++){
			if (movies[j].title < movies[maxIndex].title){
				var temp = movies[j];
				movies[j] = movies[maxIndex];
				movies[maxIndex] = temp;
			}
